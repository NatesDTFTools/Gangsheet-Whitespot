...
function buildAlphaNamesIRB(names){
  ...
  async function runExport(){
    ...
    try{
      ...
      // ---- pick save location now, BEFORE the slow encoding work below ----
      // showSaveFilePicker only works while the click that triggered this
      // function is still "fresh" (browser user-activation window, a few
      // seconds). The pixel-building loop further down can easily run past
      // that window on a long sheet, which is why the dialog previously
      // stopped appearing on big sheets — it silently failed and fell back
      // to a plain download instead. Asking here, right up front, fixes
      // that regardless of how long the sheet is. A fresh handle is
      // requested every time this function runs, so exporting again after
      // clearing the sheet (or exporting again at all) always re-prompts.
      if(window.showSaveFilePicker){
        try{
          saveHandle = await window.showSaveFilePicker({
            suggestedName: 'gang_sheet_12in.tif',
            types: [{ description: 'TIFF image', accept: {'image/tiff': ['.tif','.tiff']} }]
          });
        }catch(pickerErr){
          if(pickerErr && pickerErr.name === 'AbortError'){
            userCancelledSave = true;
            throw new Error('user cancelled save location');
          }
          console.error('Save picker failed, will fall back to browser download:', pickerErr);
        }
      }
      ...
      if(saveHandle){
        // Write to the location the user already picked, above.
        const writable = await saveHandle.createWritable();
        await writable.write(blob);
        await writable.close();
      }else{
        // Fallback for browsers without the File System Access API
        // (Firefox, Safari): triggers the normal browser download instead.
      ...
      exportingSubtext.textContent = 'Your gang sheet TIFF has been saved.';
    }catch(err){
      if(userCancelledSave){
        setStatus('Export cancelled');
        exportingTitle.textContent = 'Export cancelled';
        exportingSubtext.textContent = 'No file was saved.';
      }else{
